...
:::{.fragment}
you need to set up <span class='carrot'>**ssh (secure shell) key**</span> based authentication with
github.<br><br>
:::
...
:::{.small}
For Windows, I recommend installing `git bash` from here https://gitforwindows.org/ 
and then opening it to run `ssh-keygen`. 

Similarly, on MacOS / Linux, you should open terminal and run `ssh-keygen` if you haven't
ever created an SSH public key before. 

Once you do so, it will ask you a couple of questions like if you want to set up an additional 
password and if the file location is correct.  Hit enter twice to use the defaults (no extra password
and the `~/.ssh/id_rsa.pub` location) 

Now run `cat ~/.ssh/id_rsa.pub` to print to your screen your SSH public key.  This is what you 
will share with GitHub. 

Note that the file produced may have a name like `~/.ssh/id_ed25519.pub` — that's okay too.
:::

## how to give GitHub your ssh key 

:::{.small}
Copy the SSH key printed to the screen to your copy/paste clipboard. 

Navigate to your GitHub settings on github.com. 

Go to SSH and GPG Keys $\to$ New SSH Key.  

Paste your SSH public key in the Key text field
and give the SSH key a name (indicative of what computer you're on).
...
:::{.columns}
:::{.column width='30%'}
<img src="https://github.com/ID529/lectures2024/blob/master/graphics/hodu_tips/HoduClone.png?raw=true" alt="Hodu Tip! with two Dogs">
:::
